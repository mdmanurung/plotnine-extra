...
class PlotnineAnimation(ArtistAnimation):
    ...
    ----------
    plots :
        ggplot objects that make up the the frames of the
        animation
    interval : int
        Delay between frames in milliseconds. Defaults to 200.
    repeat_delay : int
        If the animation in repeated, adds a delay in
        milliseconds before repeating the animation.
        Defaults to `None`.
    repeat : bool
        Controls whether the animation should repeat when the
        sequence of frames is completed. Defaults to `True`.
    blit : bool
    ...
    2. The scales of all the plots should have the same limits.
       It is a good idea to create a scale (with limits) for
       each aesthetic and add them to all the plots.
    """
    ...
    def _draw_plots(
        ...
        def initialise_artist_offsets(n: int):
            ...
            n : int
                ...
                The artists for each axes are tracked
                separately.
            """
        ...
        def get_frame_artists(
            ...
            # We use offsets to mark the end of the previous
            # frame e.g ax.collections[start:]
            frame_artists = []
        ...
        def set_scale_limits(scales: list[scale]):
            ...
            ----------
            scales : list[scales]
                List of scales the have been used in building
                a ggplot object.
            """
            for sc in scales:
                ae = sc.aesthetics[0]
        ...
        def check_scale_limits(
            ...
            Raises a PlotnineError if any of the scales has
            limits that do not match those of the first
            plot/frame.
            ...
            if len(scale_limits) != len(scales):
                raise PlotnineError(
                    "All plots must have the same number of "
                    "scales as the first plot of the "
                    "animation."
                )
            ...
            for sc in scales:
                ...
                if ae not in scale_limits:
                    raise PlotnineError(
                        f"The plot for frame {frame_no} "
                        f"does not have a scale for the "
                        f"{ae} aesthetic."
                    )
                if sc.final_limits != scale_limits[ae]:
                    raise PlotnineError(
                        f"The {ae} scale of plot for frame "
                        f"{frame_no} has different limits "
                        "from those of the first frame."
                    )
        ...
        # The first ggplot creates the figure, axes and the
        # initial frame of the animation. The rest of the
        # ggplots draw onto the figure and axes created by the
        # first ggplot and they create the subsequent frames.
    ...
    def _draw_animation_plot(
        ...
        """
        from plotnine._utils.context import plot_context
