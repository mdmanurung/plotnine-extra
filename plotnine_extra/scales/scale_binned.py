...
class scale_binned(
    ...
    def get_breaks(
        ...
        if (
            self.is_empty()
            or self.breaks is False
            or self.breaks is None
        ):
            return []
        ...
        if self.breaks is True:
            ...
            breaks = breaks_extended(n=self.n_breaks)(
                self.inverse(limits)
            )
    ...
    def default_expansion(self, mult=0, add=0.5, expand=True):
        ...
        """
        return super().default_expansion(
            mult=mult, add=add, expand=expand
        )
