...
class geom_hex(geom):
    ...
    See Also
    --------
    plotnine.stat_bin_hex : The default `stat` for this `geom`.
    """
    ...
    def draw_group(
        ...
        sx = np.median(np.diff(ux)) / 2 if len(ux) > 1 else 0.5

        uy = np.sort(data["y"].unique())
        sy = np.median(np.diff(uy)) / 2 if len(uy) > 1 else sx * np.sqrt(3) / 2
